...
- Experience presets from dropdown/query param (for example `?theme=lobby`).
...
### Themes (`themes.json`)
- Presets: `lobby`, `backrooms`, `deadmall`, `poolrooms`, `stormyard`, `roman`, `inferno`, `purgatory`, `neon`, `winter`.
- Theme can override:
...
### Audio (`audio.json`)
- Ambient layers are scene-driven and synth-capable (for example `lobby_pad`, `backrooms_buzz`, `fluorescent_drift`, `far_traffic`, `night_woodland_wind`)
...
- `https://lobby.seperet.com/?theme=deadmall`
- `https://lobby.seperet.com/?theme=poolrooms`
- `https://lobby.seperet.com/?theme=stormyard`
...
- Liminal photo/video references + feature ideas: `docs/LIMINAL-REFERENCE-LIBRARY.md`
